...
struct Layer {
	...
	Layer(unsigned texture, unsigned shadow_map, int layer) {
...
#else
	std::cout << "Connected to server! localhost:32323 " << strerror(errno) << std::endl;
#endif
...
#else
	std::cout << "Started server on port 32323 " << strerror(errno) << std::endl;
#endif
...
void object_gravity_collision_function(Object* obj, float delta, float nextDelta, World& world, bool interact_with_other_objects, bool same_type) {
	...
	col -= glm::ivec4(1);
	col.x = std::max(col.x, 0);
	col.y = std::max(col.y, 0);
	col.z = std::max(col.z, 0);
	col.w = std::max(col.w, 0);
...
void App::tick(double delta) {
	...
	camera.target_zoom = std::max(float(SCREEN_HEIGHT)/float(world.height*TILE_SIZE), std::max(float(SCREEN_WIDTH) / float(world.width * TILE_SIZE), camera.target_zoom));
	camera.zoom = (1 - camera.catchup_ratio) * camera.zoom + camera.catchup_ratio * camera.target_zoom;
	camera.zoom = std::max(float(SCREEN_HEIGHT) / float(world.height * TILE_SIZE), std::max(float(SCREEN_WIDTH) / float(world.width * TILE_SIZE), camera.zoom));

	camera.target = player.pos + glm::vec2(16);
	camera.target.x = std::min(world.width * TILE_SIZE - SCREEN_WIDTH * 0.5 - SCREEN_WIDTH * 0.5 / camera.zoom, std::max(SCREEN_WIDTH * 0.5 / camera.zoom - SCREEN_WIDTH * 0.5, camera.target.x - SCREEN_WIDTH * 0.5)) + SCREEN_WIDTH*0.5;
	camera.target.y = std::min(world.height * TILE_SIZE - SCREEN_HEIGHT * 0.5 - SCREEN_HEIGHT * 0.5 / camera.zoom, std::max(SCREEN_HEIGHT * 0.5 / camera.zoom - SCREEN_HEIGHT * 0.5, camera.target.y - SCREEN_HEIGHT * 0.5)) + SCREEN_HEIGHT*0.5;
	...
	camera.position.x = std::min(world.width * TILE_SIZE - SCREEN_WIDTH * 0.5 - SCREEN_WIDTH * 0.5 / camera.zoom, std::max(SCREEN_WIDTH * 0.5 / camera.zoom - SCREEN_WIDTH * 0.5, (double)camera.position.x));
	camera.position.y = std::min(world.height * TILE_SIZE - SCREEN_HEIGHT * 0.5 - SCREEN_HEIGHT * 0.5 / camera.zoom, std::max(SCREEN_HEIGHT * 0.5 / camera.zoom - SCREEN_HEIGHT * 0.5, (double)camera.position.y));
	//}
}

...
void Player::tick(float delta, float nextDelta, World& world, app::window* window) {
	...
	collisions.x = max(collisions.x, 0);
	collisions.y = max(collisions.y, 0);
	collisions.z = max(collisions.z, 0);
	collisions.w = max(collisions.w, 0);
	...
	jumpOffsetting = max(jumpOffsetting - 2, 0);
	frictionOffsetting = max(frictionOffsetting - delta * 5.f, 0);

	if (collisions.w < 5 && frictionOffsetting == 0)
		vel.y += max(50 - jumpOffsetting * 10, 10);
	vel -= vel * min(delta * 10, 1) * glm::vec2(1.0 - frictionOffsetting, 0.2);
	if (abs(vel.x) < 0.1)
		vel.x = 0;
	if (abs(vel.y) < 0.1)
		vel.y = 0;
